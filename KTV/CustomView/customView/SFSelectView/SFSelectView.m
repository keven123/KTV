...
- (void)animationView{
    ...
    if (visiable) {
        ...
        CGPoint newPoint=[self convertPoint:CGPointZero toView:window];
        imageV=[[UIImageView alloc]initWithFrame:CGRectMake(newPoint.x-10, newPoint.y+self.bounds.size.height+12, self.frame.size.width*2.5, self.items.count*myTableView.rowHeight+4)];
        imageV.image=[UIImage imageNamed:@"selectView_bg"];
        myTableView.frame = CGRectMake(0,5,imageV.frame.size.width,imageV.frame.size.height);
     ...
     [UIView animateWithDuration:2 delay:0.0 usingSpringWithDamping:0.8 initialSpringVelocity:0.2 options:UIViewAnimationOptionOverrideInheritedCurve animations:^{
         imageV.frame=CGRectMake(imageV.frame.origin.x, imageV.frame.origin.y, 0, 0);
         imageV.alpha=0.0;
